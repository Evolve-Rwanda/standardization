...
public class ColumnInitializer {
    ...
    public List<Column> getTableOfColumnValueOptionsColumnList(){
        ...
        tocvoColumnList.add(new Column(tableName, "option_value", 2,"VARCHAR", 100, 0, null, false, false, false, false, false, false, null, null, null, "", "This holds one of the possible values of this particular column. It is from these values that a choice for a value of a particular column will be made through the UI.", timestamp, null));
    ...
    public List<Column> getTableOfColumnInputMarkupElementColumnList(){
        ...
        tableName = !dbDocSchemaName.isEmpty() ? (dbDocSchemaName + "." + tableName): tableName;
        String timestamp = DateTime.getTimeStamp();
        tocimeColumnList.add(new Column(tableName, "column_id", 1, "SMALLINT", 7, 0, null, false, false, false, true, false, false, null, null, null, "", "The primary key of this table which is also a foreign key in the table of columns, although the foreign key constraint may not be explicitly enforced.", timestamp, null));
        tocimeColumnList.add(new Column(tableName, "tag_name", 2,"VARCHAR", 100, 0, null, false, false, false, false, false, false, null, null, null, "", "This is the name used to form the data input tag. It is specified just after the opening tag's left pointing angle bracket.", timestamp, null));
        tocimeColumnList.add(new Column(tableName, "type_attribute_value", 3,"VARCHAR", 20, 0, null, true, false, false, false, false, false, null, null, null, "", "Some html form data input tags are only distinguished by a type attribute. This is where we specify the value of the distinguishing attribute.", timestamp, null));
        tocimeColumnList.add(new Column(tableName, "name_attribute_value", 4,"VARCHAR", 100, 0, null, false, false, false, false, false, false, null, null, null, "", "This holds name of the html form data-input tag that is added through a similarly named attribute.", timestamp, null));
        tocimeColumnList.add(new Column(tableName, "is_mutually_exclusive", 5,"VARCHAR", 10, 0, null, false, false, false, false, false, false, null, null, null, "", "This column records whether a single item among the options should be selected. it also records whether multiple items should selected", timestamp, null));
